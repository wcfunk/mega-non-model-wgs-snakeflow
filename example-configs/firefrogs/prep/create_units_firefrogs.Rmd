---
title: "Create units for Ascaphus RAPID project, aka, FireFrogs"
author: "Christen Bossu, modified by W. Chris Funk"
date: "4/22/2025, modified by W. Chris Funk on 11/07/2025"
...
```
ls -l --color=never data/fastqs/*/*/*.fq.gz > firefrogs.fastq_listing.txt
```
...
files <- read_table("firefrogs.fastq_listing.txt", col_names = FALSE) %>%
  select(X8, X4) %>%
  mutate(kb = X4/1000) %>%
  rename(fq = X8) %>%
  select(fq, kb) %>%
  mutate(base = basename(fq)) %>% 
  separate(base,into=c("sample_id","num","flowcell","lane","read"),sep="[_.]")
...
```sh
for i in data/fastqs/*/*/*.fq.gz; do zcat $i | awk -v f=$i 'BEGIN {OFS="\t"} NR==1 {print f, $1; exit}'; done | awk '!/Undetermined/' > firefrogs.novo.seq-tags-per-path.tsv
```
The results of that have been put into:
```
example-configs/firefrogs/prep/firefrogs.novo.seq-tags-per-path.tsv
```
And we can make it nice like this:
```{r}
seq_ids <- read_tsv("firefrogs.novo.seq-tags-per-path.tsv", col_names = c("fq", "id")) %>%
  separate(
    id, 
...
# 2) Join header-derived flowcell/lane and coalesce into single columns
seq_ids_renamed <- seq_ids %>%
  rename(flowcell_hdr = flowcell, lane_hdr = lane)  # avoid join collisions

files_joined <- files %>%
  left_join(seq_ids_renamed, by = "fq") %>%
  mutate(
    # prefer header-derived; fall back to filename-derived if header missing
    flowcell = dplyr::coalesce(flowcell_hdr, flowcell),
    lane     = dplyr::coalesce(lane_hdr, lane)
  ) %>%
  select(-flowcell_hdr, -lane_hdr)
...
# 4) Wide pivot -> fq1, fq2, kb1, kb2 (now they’ll be scalars, not lists)
...
flowcell_to_library <- c(
  "HHNVFDSX7" = "novogene_202402",				
  "H3WCCDSXC" = "novogene_20240510"
)

files_wide <- files_wide %>%
  mutate(library = recode(flowcell, !!!flowcell_to_library, .default = "novogene_20240524"))
...
write_tsv(units, "firefrogs.units.tsv")
...
```{r}
#Ascaphus truei genome: GCA_036426205.1_UCB_Atru_1.0_genomic.fna which I indexed and named genome.fasta.fai

# all 22 A. truei chromosomes are > 11 Mb and all scaffolds are < 1 Mb.
# so we will use these length thresholds for separating chromosomes and scaffolds.
fai <- read_tsv(
  "../../../resources/genome.fasta.fai", col_names = c("chrom", "len", "x1", "x2", "x3")) %>%
...
# get rid of small scaffolds (< 50 Mb):
fai <- read_tsv(
  "../../../resources/genome.fasta.fai", col_names = c("chrom", "len", "x1", "x2", "x3")) %>% filter(len>50000) %>% 
  dplyr::select(-starts_with("x")) %>%
  mutate(cumlen = cumsum(len))
...
scaff_groups %>%
  ...
  summarise(
    tot_bp = sum(len),
    num_scaff = n()
  ) %>% write_tsv("firefrogs.scaffold_groups.summary.tsv")
