...
date: "4/22/2025, modified by W. Chris Funk on 06/16/2025"
...
```
ls -l --color=never data/fastqs/wm152/*gz > wm152.fastq_listing.txt
```

I had to make separate lists because sample ID names have different numbers of "_" in sample ID names. Total pain in the ass, so in future, need to make sure that sample ID names have the SAME number of "_" in their names.

```{r, message=FALSE, warning=FALSE}
library(tidyverse)

files3underscores <- read_table("wm152-3underscores.fastq_listing.txt", col_names = FALSE) %>%
  select(X9, X5) %>%
  mutate(kb = X5/1000) %>%
  rename(fq = X9) %>%
  select(fq, kb) %>%
...
files4underscores <- read_table("wm152-4underscores.fastq_listing.txt", col_names = FALSE) %>%
  select(X9, X5) %>%
  mutate(kb = X5/1000) %>%
  rename(fq = X9) %>%
  select(fq, kb) %>%
...
files5underscores <- read_table("wm152-5underscores.fastq_listing.txt", col_names = FALSE) %>%
  select(X9, X5) %>%
  mutate(kb = X5/1000) %>%
  rename(fq = X9) %>%
  select(fq, kb) %>%
...
files6underscores <- read_table("wm152-6underscores.fastq_listing.txt", col_names = FALSE) %>%
  select(X9, X5) %>%
  mutate(kb = X5/1000) %>%
  rename(fq = X9) %>%
  select(fq, kb) %>%
...
files <- bind_rows(files3underscores, files4underscores, files5underscores, files6underscores)
...
onto the path:
```sh
for i in data/fastqs/wm152/*gz; do zcat $i | awk -v f=$i 'BEGIN {OFS="\t"} NR==1 {print f, $1; exit}'; done | awk '!/Undetermined/' > wm152.novo.seq-tags-per-path.tsv
```
...
```{r}
seq_ids <- read_tsv("wm152.novo.seq-tags-per-path.tsv", col_names = c("fq", "id")) %>%
  separate(
    id, 
...
files_wide <- files %>%
  ...
  mutate(
    sample = sample_id,
    .before = sample_id
  ) %>% mutate(library=if_else(flowcell=="HCVC5DSX7","Plate1",if_else(flowcell=="22HM5LLT4","Plate2",if_else(flowcell=="22HVG3LT4","Plate3","Plate4"))))
...
write_tsv(units, file = "wm152.Novoseq.units.correct.tsv")
```
