---
title: "Create units for monarch-test-2025"
author: "Christen Bossu, modified by W. Chris Funk"
date: "4/22/2025, modified by W. Chris Funk on 06/10/2025"
...
```
ls -l  data/WAVI00*_Novogene_122024/01.RawData/W*/*gz > WAVI001_003.fastq_listing.txt
```

Chris' modified versions (had to make separate lists because sample ID names have different numbers of "_" in sample ID names:
```
ls -l --color=never data/fastqs/test/Adult*gz > monarch-test-2025-Adult.fastq_listing.txt

ls -l --color=never data/fastqs/test/EP*gz > monarch-test-2025-EP.fastq_listing.txt

ls -l --color=never data/fastqs/test/KVP*gz > monarch-test-2025-KVP.fastq_listing.txt
```

```{r, message=FALSE, warning=FALSE}
library(tidyverse)

filesAdult <- read_table("monarch-test-2025-Adult.fastq_listing.txt", col_names = FALSE) %>%
  select(X9, X5) %>%
  mutate(kb = X5/1000) %>%
  rename(fq = X9) %>%
  select(fq, kb) %>%
...
filesEP <- read_table("monarch-test-2025-EP.fastq_listing.txt", col_names = FALSE) %>%
  select(X9, X5) %>%
  mutate(kb = X5/1000) %>%
  rename(fq = X9) %>%
  select(fq, kb) %>%
...
filesKVP <- read_table("monarch-test-2025-KVP.fastq_listing.txt", col_names = FALSE) %>%
  select(X9, X5) %>%
  mutate(kb = X5/1000) %>%
  rename(fq = X9) %>%
  select(fq, kb) %>%
...
filesAdult <- filesAdult %>% mutate(source = "Adult")
filesEP    <- filesEP %>% mutate(source = "EP")
filesKVP   <- filesKVP %>% mutate(source = "KVP")

files <- bind_rows(filesAdult, filesEP, filesKVP)
...
onto the path:
```sh
for i in data/fastqs/test/*gz; do zcat $i | awk -v f=$i 'BEGIN {OFS="\t"} NR==1 {print f, $1; exit}'; done | awk '!/Undetermined/' > monarch-test-2025.novo.seq-tags-per-path.tsv
```
The results of that have been put into:
```
example-configs/WAVI_May_2025/prep/inputs/seq-tags-per-path.tsv
```
And we can make it nice like this:
```{r}
seq_ids <- read_tsv("monarch-test-2025.novo.seq-tags-per-path.tsv", col_names = c("fq", "id")) %>%
  separate(
    id, 
...
files_wide <- files %>%
  ...
  #mutate(lane=1,flowcell="highseq") %>% 
  pivot_wider(
    values_from = c(fq, kb),
    names_from = read,
    names_sep = ""
  ...
  mutate(
    sample = sample_id,
    .before = sample_id
  ) %>% mutate(library=if_else(flowcell=="HCVC5DSX7","Plate1",if_else(flowcell=="HCVC5DSXX","Plate2","Plate3")))
...
```{r}
 ...
 units<- files_wide %>% group_by(sample) %>%
  mutate(unit = 1:n(), .after = sample) %>%
  ungroup() %>%
  mutate(
    barcode = str_c(sample_id, library, lane, sep = "-")
  ) %>%
  dplyr::select(sample, unit, library, flowcell, platform, lane, sample_id, barcode, fq1, fq2, kb1, kb2) 

write_tsv(units, file = "monarch-test-2025.Novoseq.units.correct.tsv")
```
